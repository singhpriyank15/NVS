...
------------------- Transaction present for mismatched SFIDs b/w ODS-Veeva: Update SF_ID in ODS & delete upd_sf_id in VEEVA -----------------------
...
spark.sql("create table if not exists all_all_r_all_bkp.ods_contacts_addr_case1_11_2 like ALL_ALL_B_USA_CRMODS.ods_contacts_addr")
spark.sql("insert into all_all_r_all_bkp.ods_contacts_addr_case1_11_2 select * from ALL_ALL_B_USA_CRMODS.ods_contacts_addr")
...
select upd_data_id,upd_sf_id,id from final_data where sf_id_check = true and (call_cnt_upd <> 0 or event_cnt_upd <> 0) and (call_cnt_veeva = 0 and event_cnt_veeva = 0)""").createOrReplaceTempView("final_data")

-- Delete IDS in VEEVA having No TRANSACTION   

val df = spark.sql("select * from final_data")

df.select("upd_sf_id").repartition(1).write.option("header","true").option("sep",",").mode("append").csv("/user/sys_evico_prd_atlas/sop")  -- Give your local HDFS path. Share & use the file for Dataloader
